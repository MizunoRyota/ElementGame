...
void EnemyStateChoseAttack::ChoseNextAttackPhaseOne()
{
	...
	if (checkDistance >= LONG_RANGE)
	{
		...
		if (nextAttackType < ATTACK_RAITO6)         SetEnemyAttackState(EnemyStateKind::STATE_FIREATTACK);  
		else if (nextAttackType < ATTACK_RAITO10)    SetEnemyAttackState(EnemyStateKind::STATE_WATERATTACK);  
	}
...
void EnemyStateChoseAttack::ChoseNextAttackPhaseTwo()
{
	...
	if (checkDistance >= LONG_RANGE)
	{
		...
		if (nextAttackType < ATTACK_RAITO2)         SetEnemyAttackState(EnemyStateKind::STATE_FIREATTACK);
		else if (nextAttackType < ATTACK_RAITO8)    SetEnemyAttackState(EnemyStateKind::STATE_JUMPATTACK);
		else if(nextAttackType < ATTACK_RAITO9)  	SetEnemyAttackState(EnemyStateKind::STATE_WINDATTACK);
		else if (nextAttackType < ATTACK_RAITO10)   SetEnemyAttackState(EnemyStateKind::STATE_FLOAT);		 
	}
	else if (checkDistance >= SHORT_RANGE)
	{
...
void EnemyStateChoseAttack::ChoseNextAttackPhaseThree()
{
	...
	if (checkDistance >= LONG_RANGE)
	{
		...
		if (nextAttackType < ATTACK_RAITO6)			SetEnemyAttackState(EnemyStateKind::STATE_JUMPATTACK);
		else if (nextAttackType < ATTACK_RAITO8)    SetEnemyAttackState(EnemyStateKind::STATE_WATERATTACK);
		else if (nextAttackType < ATTACK_RAITO9)    SetEnemyAttackState(EnemyStateKind::STATE_FLOAT);		
		else if (nextAttackType < ATTACK_RAITO10)	SetEnemyAttackState(EnemyStateKind::STATE_WINDATTACK);
	}
	else if (checkDistance >= SHORT_RANGE)
	{
