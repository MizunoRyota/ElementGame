...
void PlayerMove::Update()
{
	...
	moveVec = VGet(0, 0, 0);
	UpdateMoveParameterWithPad( upMoveVec, leftMoveVec, moveVec);
	DecreaseDashEnergy();
	MoveAngle(ObjectAccessor::GetObjectAccessor().GetCameraDirection());
	Move(moveVec);
}

// パッド/キーボード入力から移動ベクトルを算出
void PlayerMove::UpdateMoveParameterWithPad( VECTOR& upMoveVec, VECTOR& leftMoveVec, VECTOR& moveVec)
{
	upMoveVec = VSub(ObjectAccessor::GetObjectAccessor().GetCameraTarget(), ObjectAccessor::GetObjectAccessor().GetCameraPosition());
	upMoveVec.y = 0.0f;
	...
	if ((ObjectAccessor::GetObjectAccessor().GetInputNowFrame() & PAD_INPUT_LEFT) || (CheckHitKey(KEY_INPUT_A) != 0))
	{
		moveVec = VAdd(moveVec, leftMoveVec);
		isPressMoveButton = true;
	}
	else if ((ObjectAccessor::GetObjectAccessor().GetInputNowFrame() & PAD_INPUT_RIGHT) || (CheckHitKey(KEY_INPUT_D) != 0))
	{
		moveVec = VAdd(moveVec, VScale(leftMoveVec, -1.0f));
		isPressMoveButton = true;
	}

	if ((ObjectAccessor::GetObjectAccessor().GetInputNowFrame() & PAD_INPUT_UP) || (CheckHitKey(KEY_INPUT_W) != 0))
	{
		moveVec = VAdd(moveVec, upMoveVec);
		isPressMoveButton = true;
	}
	else if ((ObjectAccessor::GetObjectAccessor().GetInputNowFrame() & PAD_INPUT_DOWN) || (CheckHitKey(KEY_INPUT_S) != 0))
	{
