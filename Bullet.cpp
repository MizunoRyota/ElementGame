...
void Bullet::InitializeHoming(const VECTOR& pos, const VECTOR& dir, const float& speed, std::function<VECTOR()> targetGetter, float homingDuration, float turnSpeedRad, int effectTypeIndex)
{
    Initialize(pos, dir, speed, effectTypeIndex);
    bullet_is_Homing = true;
    bullet_homingTime = homingDuration;
...
void Bullet::Move()
{
    ...
    if (bullet_is_Homing && bullet_target_getter)
    {
        ...
        VECTOR toTarget = VSub(targetPos, bullet_position);
        float distSq = VDot(toTarget, toTarget);
        if (distSq > 1e-6f)
        {
            VECTOR desiredDir = VNorm(toTarget);
